...
Use `native-mcp` when the server already exists and only needs to be connected to Enough. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.enough/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.enough/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.enough/config.json` using the `native-mcp` skill, or
...
### Enough cannot see the deployed server

The server-building part may be correct while the Enough `config.json` is not. Load the `native-mcp` skill and configure the server in `~/.enough/config.json`, then restart Hermes.
