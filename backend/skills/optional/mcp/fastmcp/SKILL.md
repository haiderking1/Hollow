...
Use `native-mcp` when the server already exists and only needs to be connected to Hollow. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hollow/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hollow/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.hollow/config.json` using the `native-mcp` skill, or
...
### Hollow cannot see the deployed server

The server-building part may be correct while the Hollow `config.json` is not. Load the `native-mcp` skill and configure the server in `~/.hollow/config.json`, then restart Hermes.
