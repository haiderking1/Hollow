...
version: 1.0.0
author: arceus77-7, enhanced by Hollow
license: MIT
...
- `tmux` available for stable authenticated sessions during Hollow bash calls (desktop app flow only)
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HOLLOW_HOME:-~/.hollow}/.env` (the skill will prompt for this on first load).
...
Hollow bash commands are non-interactive by default and can lose auth context between calls.
