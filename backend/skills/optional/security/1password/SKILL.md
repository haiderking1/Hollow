...
version: 1.0.0
author: arceus77-7, enhanced by Enough
license: MIT
...
- `tmux` available for stable authenticated sessions during Enough bash calls (desktop app flow only)
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ENOUGH_HOME:-~/.enough}/.env` (the skill will prompt for this on first load).
...
Enough bash commands are non-interactive by default and can lose auth context between calls.
