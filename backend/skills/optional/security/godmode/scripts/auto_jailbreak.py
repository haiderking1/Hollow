...
finds what works, and locks it in by writing config.json + prefill.json.
...
def _write_config(system_prompt: str = None, prefill_file: str = None):
    """Write jailbreak settings to config.json (merges, doesn't overwrite)."""
    cfg = {}
...
def auto_jailbreak(model=None, base_url=None, api_key=None,
    ...
    4. Locks in the winning combo (writes config.json + prefill.json)
    ...
    if not model:
        return {"success": False, "error": "No model specified and couldn't read config.json"}
    if not base_url:
...
def undo_jailbreak(verbose=True):
    """Remove jailbreak settings from config.json and delete prefill.json."""
    if CONFIG_PATH.exists():
        try:
            with open(CONFIG_PATH) as f:
                cfg = yaml.safe_load(f) or {}
