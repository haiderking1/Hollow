...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Enough
license: MIT
...
The script reads `${ENOUGH_HOME:-~/.enough}/.env` for two optional defaults:
...
Helper script: `~/.enough/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.enough/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ENOUGH_HOME:-~/.enough}/.env`.
...
```bash
python3 ~/.enough/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.enough/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
