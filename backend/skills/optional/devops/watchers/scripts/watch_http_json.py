...
Usage (via cron with --no-agent):
    ...
    cron (Hermes-only — use bash + cron on host) create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ENOUGH_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Enough-Watcher/1.0"})
