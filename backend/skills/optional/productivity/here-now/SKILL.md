...
- Skill helper paths:
  - `${HOLLOW_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HOLLOW_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HOLLOW_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${HOLLOW_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
