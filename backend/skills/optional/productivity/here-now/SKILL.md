...
- Skill helper paths:
  - `${ENOUGH_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ENOUGH_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ENOUGH_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${ENOUGH_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
