...
| File operations | Generic instructions | file tools (read_file, write_file, bash, grep) (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Enough repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
