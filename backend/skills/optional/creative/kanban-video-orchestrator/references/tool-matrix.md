...
Maps each role archetype to the Hollow skills it should `always_load` and the
...
### Visual / rendering skills (`Hollow skills tree (bundled/)creative/`)
...
### Design / pre-production skills (`Hollow skills tree (bundled/)creative/`)
...
### Audio / media skills (`Hollow skills tree (bundled/)creative/` + `skills/media/`)
...
### Kanban infrastructure (`Hollow skills tree (bundled/)devops/`)
...
## Built-in Hollow tools for media review

These are native Hollow tools — not invoked via terminal but through their own
...
key is present in `${HOLLOW_HOME:-~/.hollow}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${HOLLOW_HOME:-~/.hollow}/.env` → environment variable.
