...
Maps each role archetype to the Enough skills it should `always_load` and the
...
### Visual / rendering skills (`Enough skills tree (bundled/)creative/`)
...
### Design / pre-production skills (`Enough skills tree (bundled/)creative/`)
...
### Audio / media skills (`Enough skills tree (bundled/)creative/` + `skills/media/`)
...
### Kanban infrastructure (`Enough skills tree (bundled/)devops/`)
...
## Built-in Enough tools for media review

These are native Enough tools — not invoked via terminal but through their own
...
key is present in `${ENOUGH_HOME:-~/.enough}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${ENOUGH_HOME:-~/.enough}/.env` → environment variable.
