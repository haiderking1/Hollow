...
```bash
mkdir -p ~/.enough/cache/darwinian-evolver && cd ~/.enough/cache/darwinian-evolver
...
```bash
cd ~/.enough/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.enough/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.enough/skills/research/darwinian-evolver
DE_DIR=~/.enough/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.enough/skills/...` are user-side and fine.
