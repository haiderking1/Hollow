...
```bash
mkdir -p ~/.hollow/cache/darwinian-evolver && cd ~/.hollow/cache/darwinian-evolver
...
```bash
cd ~/.hollow/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hollow/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hollow/skills/research/darwinian-evolver
DE_DIR=~/.hollow/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.hollow/skills/...` are user-side and fine.
