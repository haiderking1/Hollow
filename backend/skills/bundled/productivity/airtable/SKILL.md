...
4. Store the token in `${HOLLOW_HOME:-~/.hollow}/.env` (or via edit `~/.hollow/config.json` or `/connect` in the TUI`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Hollow. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${HOLLOW_HOME:-~/.hollow}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
