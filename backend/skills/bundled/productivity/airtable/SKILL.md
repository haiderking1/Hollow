...
4. Store the token in `${ENOUGH_HOME:-~/.enough}/.env` (or via edit `~/.enough/config.json` or `/connect` in the TUI`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Enough. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${ENOUGH_HOME:-~/.enough}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
