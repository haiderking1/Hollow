...
- A long-running process (Hollow UI, Hollow UI) misbehaves and you can't restart it
...
For long-lived processes: messaging gateway (Hermes-only), Hollow UI, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source the Hollow project/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Hollow you have two practical options:
...
```json
{
  "name": "Attach to Hollow",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "the Hollow project" }
  ]
}
```
...
## Debugging Hollow-specific Processes
...
### `Hollow UI` subprocess (spawned by `enough`)
The gateway runs as a child of the Node TUI. Options:

**A. Source-edit the gateway:**
```python
# Hollow UI/server.py near the top of serve()
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Hollow swarm workers, debug one process at a time.
