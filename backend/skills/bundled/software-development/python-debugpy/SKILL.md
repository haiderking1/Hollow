...
metadata:
  hermes:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, enough]
---
...
- A long-running process (Enough TUI, Enough TUI) misbehaves and you can't restart it
...
`go test` and pytest both support this:
...
For long-lived processes: messaging gateway (Hermes-only), Enough TUI, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source the Enough project/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Enough you have two practical options:
...
```json
{
  "name": "Attach to Enough",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "the Enough project" }
  ]
}
```
...
## Debugging Enough-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `enough` normally. Control returns to your terminal at the pause point.

### `Enough TUI` subprocess (spawned by `enough`)
The gateway runs as a child of the Node TUI. Options:

**A. Source-edit the gateway:**
```python
# Enough TUI/server.py near the top of serve()
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
Start `enough`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Enough swarm workers, debug one process at a time.
